...
func VerificationMessage(to, link string) Message {
	...
	html := fmt.Sprintf(
		...
		link)
	return Message{To: to, Subject: "Confirm your email — Ticket Price Watcher", HTMLBody: html, TextBody: text}
}

// PasswordResetMessage carries a one-time link to choose a new password.
func PasswordResetMessage(to, link string) Message {
	text := fmt.Sprintf(
		"Reset your Ticket Price Watcher password:\n\n%s\n\n"+
	...
	html := fmt.Sprintf(
		...
		link)
	return Message{To: to, Subject: "Reset your password — Ticket Price Watcher", HTMLBody: html, TextBody: text}
}
